...
O arquivo app\Modules\ProductManagement\Infrastructure\Config\Colors.php contém as cores padrões do sistema.

</context>

<goal>
  Criar um comando que sincronize as cores padrões ao banco de dados.
</goal>


<instructions>
  1. Crie um comando que sincronize as cores padrões descritas em app\Modules\ProductManagement\Infrastructure\Config\Colors.php à tabela categories do banco de dados.

  2. Adicione esse comando ao arquivo scripts\sync.sh
  
  3. Documente a implementação no arquivo docs\changelog\05_product_management\02_sync_products_list\implementação.md. No mesmo arquivo dê sugestões de mensagem para o commit usando boas práticas de Git.
